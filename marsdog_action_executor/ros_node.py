    ...
    class ActionExecutorNode(Node):
        ...
        def __init__(self) -> None:
            ...
            if self.get_parameter("navigation_enabled").value:
                ...
                self._mobility_adapter = BehaviorMobilityAdapter(
                    ...
                    random_navigation_bounds=navigation_config.get(
                        "random_navigation_bounds", {}
                    ),
                    random_navigation_region=navigation_config.get(
                        "random_navigation_region", {}
                    ),
                    random_navigation_exclude_waypoints=navigation_config.get(
        ...
        async def _execute_behavior(self, goal_handle):
            ...
            if (
                ...
                # Use a shorter timeout for random navigation within the
                # small polygon; keep the full timeout for fixed-waypoint
                # navigation that may cover longer distances.
                is_random_nav = (
                    canonical
                    in self._mobility_adapter._random_navigation_behaviors
                )
                if is_random_nav:
                    random_region = self._config.navigation_config.get(
                        "random_navigation_region", {}
                    )
                    nav_timeout = float(
                        random_region.get("result_timeout_sec")
