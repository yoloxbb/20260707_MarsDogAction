...
Usage::
    ...
    uv run python -m marsdog_action_executor.standalone_demo eatNormally
    uv run python -m marsdog_action_executor.standalone_demo come_to_owner 42
"""
...
def main() -> None:
    ...
    if not ctx.is_valid:
        ...
        avail = sorted(loader.get_behavior_names())
    ...
    # ── Evaluate ──────────────────────────────────────────────────────────
    br = evaluator.evaluate(
        ctx,
        stage_results,
        template.get("success_condition"),
    )
    print()
